...
Note that it doesn't seem to be the brick impact that triggers the sound, but the scoring counter increment.  One strike on a Yellow Brick cause one tone for one point added to the score.  One strike on a Red Brick causes 7 tones as the game adds 7 individual points to the score.
